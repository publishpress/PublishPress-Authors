...
class CMB2 extends CMB2_Base
{
    ...
    protected $mb_defaults = [
        ...
        */
        ...
        'tab_group' => '',
        // Tab-group identifier, enables options page tab navigation.
     ...
     * @since 2.2.3
     */
    protected $tax_metaboxes_to_remove = [];
    ...
    public function __construct($config, $object_id = 0)
    {
        ...
        add_action("cmb2_init_hookup_{$this->cmb_id}", ['CMB2_hookup', 'maybe_init_and_hookup']);

        // Hook in the rest api functionality.
        add_action("cmb2_init_hookup_{$this->cmb_id}", ['CMB2_REST', 'maybe_init_and_hookup']);
    }
    ...
    {
        $classes = ['cmb2-wrap', 'form-table'];

        // Use the callback to fetch classes.
        if ($added_classes = $this->get_param_callback_result('classes_cb')) {
            $added_classes = is_array($added_classes) ? $added_classes : [$added_classes];
     ...
     */
    public function box_types($fallback = [])
    {
    ...
    public function current_object_type()
    {
        ...
        if (
            in_array($pagenow, ['admin.php', 'admin-post.php'], true)
    ...
    public function add_field(array $field, $position = 0)
    {
        ...
        switch ($field['type']) {
            ...
            case 'file_list':
                ...
                add_filter('wp_prepare_attachment_for_js', ['CMB2_Type_File_Base', 'prepare_image_sizes_for_js'], 10,
                    3);
                break;
            ...
            case 'group':
                if (empty($field['render_row_cb'])) {
                    $field['render_row_cb'] = [$this, 'render_group_callback'];
                }
                break;
            case 'colorpicker':
                ...
                add_action('admin_enqueue_scripts', ['CMB2_Type_Colorpicker', 'dequeue_rgba_colorpicker_script'], 99);
                break;
        }
    ...
    protected function define_field_column(array $field)
    {
        ...
        $column = is_array($field['column']) ? $field['column'] : [];

        $field['column'] = wp_parse_args($column, [
    ...
    public function add_group_field($parent_field_id, array $field, $position = 0)
    {
        ...
        return [$parent_field_id, $field['id']];
    }
    ...
    {
        if ($position) {
            CMB2_Utils::array_insert($fields, [$field['id'] => $field], $position);
        } else {
    ...
    public function get_field_ids($field_id, $parent_field_id = '')
    {
        ...
        return false === $sub_field_id ? false : [$field_id, $sub_field_id];
    }
    ...
    {
        return $this->prop('context') && in_array($this->prop('context'),
                ['form_top', 'before_permalink', 'after_title', 'after_editor'], true);
    }
